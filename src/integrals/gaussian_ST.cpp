  ...
  for(int i = 0; i < o1->getnterms(); i++) {
	...
	std::exp(-o1->getalpha(i) * o2->getalpha(j) / zeta *
		 ((c1[0] - c2[0]) * (c1[0] - c2[0]) +
		  (c1[1] - c2[1]) * (c1[1] - c2[1]) +
		  (c1[2] - c2[2]) * (c1[2] - c2[2])));
      // Compute [e|0] integrals.
      std::map<std::array<int, 3>, double> e0;
      std::map<std::array<int, 6>, double> ap;
      for(int k = 0; k <= pow1[0] + pow2[0] + 2; k++) {
	for(int l = 0; l <= pow1[1] + pow2[1] + 2; l++) {
	  for(int m = 0; m <= pow1[2] + pow2[2] + 2; m++) {
	    std::array<int, 6> index = {k, l, m, 0, 0, 0};
	    double res = compute_ap(*&index, *&ap, zeta,
				    c1[0], c1[1], c1[2],
				    Px, Py, Pz, K);
	    std::array<int, 3> ind2 = {k, l, m};
	    e0[ind2] = res;
	  }
	}
      }
